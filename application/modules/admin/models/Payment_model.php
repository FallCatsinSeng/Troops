...
class Payment_model extends CI_Model {
    ...
    public function set_payment_status($id, $status, $order)
    {
        ...
        $this->db->where('id', $id)->update('payments', array('payment_status' => $status));

        // Jika pembayaran dikonfirmasi (status = 2)
        if ($status == 2) {
            // Update order status jadi "Telah dikonfirmasi"
            $data = array(
                'order_status' => 2
            );
            $this->db->where('id', $order)->update('orders', $data);
        }
        // Jika pembayaran ditolak/tidak ada (status = 3)
        else if ($status == 3) {
            $this->db->where('id', $order)->update('orders', array('order_status' => 5)); // Dibatalkan
        }

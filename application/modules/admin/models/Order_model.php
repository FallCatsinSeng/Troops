...
class Order_model extends CI_Model
{
    ...
    {
        $this->db->select('o.id, o.order_number, o.order_date, o.order_status, o.payment_method, o.total_price, o.total_items, c.name AS coupon, cu.name AS customer, p.payment_status')
    ...
    {
        return $this->update($order, array('order_status' => $status));
    }
    ...
    {
        // Jika pembayaran dikonfirmasi, ubah status jadi "Telah dikonfirmasi"
    ...
    {
        // Jika ada konfirmasi pembayaran dan status masih 1 (menunggu pembayaran)
        if (isset($data['payment_confirmed']) && $data['payment_confirmed'] == 1) {
            $order = $this->get_order_data($id);
            if ($order && $order->order_status == 1) {
                $data['order_status'] = 2; // Status "Telah dikonfirmasi"
            }
        }
    ...
    public function confirm_payment($id)
    {
        $data = array(
            'payment_confirmed' => 1,
            'order_status' => 2 // Status "Telah dikonfirmasi"
        );

        return $this->update_order($id, $data);
    }

    public function get_shipped_orders_with_tracking()
    {
